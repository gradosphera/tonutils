...
from ton_core import Address, AddressLike, Binary, Cell, PublicKey
...
class SeqnoGetMethod(ContractProtocol[t.Any]):
...
async def get_public_key_get_method(
    ...
    return PublicKey(r[0])


class GetPublicKeyGetMethod(ContractProtocol[t.Any]):
...
class GetSubwalletIDGetMethod(ContractProtocol[t.Any]):
...
class IsPluginInstalledGetMethod(ContractProtocol[t.Any]):
...
class GetPluginListGetMethod(ContractProtocol[t.Any]):
    """Mixin for the ``get_plugin_list`` get-method."""

    async def get_plugin_list(self) -> list[t.Any]:
...
class IsSignatureAllowedGetMethod(ContractProtocol[t.Any]):
...
class GetExtensionsGetMethod(ContractProtocol[t.Any]):
...
class ProcessedGetMethod(ContractProtocol[t.Any]):
...
class GetLastCleanTimeGetMethod(ContractProtocol[t.Any]):
...
class GetTimeoutGetMethod(ContractProtocol[t.Any]):
