...
- Rotating the signing key of a Telegram wallet (WalletTg)
...
| `WalletTg` | Telegram | 255 | Telegram-created wallets with on-chain signing-key rotation |
...
## Telegram Wallet (WalletTg)

`WalletTg` is the wallet contract used by Telegram. Its account code is a small immutable trampoline that runs shared logic stored in the blockchain config, so every `WalletTg` uses the same latest contract version, updated by validator voting. It supports up to 255 messages per transaction and adds one capability the standard wallets do not have: rotating its signing key on-chain. Its default `subwallet_id` is network-specific and resolved automatically from the client's network.

### Rotate Signing Key

`change_public_key_from_mnemonic()` replaces the wallet's on-chain signing key. The request is signed twice — by the current key, which authorizes the change, and by the new key, which proves ownership of the key being installed. The wallet **address does not change**: the balance and history stay in place, only the key that controls them changes.
...
# Current mnemonic controlling the wallet
MNEMONIC = "word1 word2 word3 ..."
...
async def main() -> None:
    ...
    # Open the wallet with its current mnemonic
...
<Warning>
After the rotation is accepted on-chain, the old mnemonic no longer controls the wallet. Reopen it with the new mnemonic using `from_address_and_mnemonic()` — `from_mnemonic()` would derive a different, empty address.
</Warning>

### Open by Address

After a key rotation, the wallet's address no longer derives from its mnemonic. Open it by its known address instead: `from_address_and_mnemonic()` binds the current key to the address and verifies the key actually controls it.
...
# Current mnemonic controlling that wallet
MNEMONIC = "word1 word2 word3 ..."
...
async def main() -> None:
    ...
    # Open the wallet by address with the current mnemonic
    # Verifies the key controls the address before returning
    # Returns: (wallet, public_key, private_key, mnemonic)
...
<Note>
`from_address_and_private_key()` is the private-key equivalent of `from_address_and_mnemonic()`, and `change_public_key()` rotates to a `PrivateKey` directly instead of a mnemonic.
</Note>
